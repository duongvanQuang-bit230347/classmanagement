...
<body>
    <div class="container">
        <div class="card">
            <h2 class="mb-4 text-center text-2xl font-bold text-[#ff6f61]">Sửa Thời Khóa Biểu</h2>
            <c:choose>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="roomId" class="form-select" required>
                                <c:forEach var="room" items="${rooms}">
                                    <option value="${room.id}" ${schedule.roomId == room.id ? 'selected' : ''}>${room.roomNumber} (${room.status})</option>
                                </c:forEach>
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="userId" class="form-select" required>
                                <c:forEach var="user" items="${users}">
                                    <option value="${user.id}" ${schedule.userId == user.id ? 'selected' : ''}>${user.fullName} (${user.role})</option>
                                </c:forEach>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Ngày</label>
                            <input type="text" name="scheduleDate" id="scheduleDate" class="form-control" value="${schedule.scheduleDate}" required>
                        </div>
                        <div class="mb-3">
                ...
                <c:otherwise>
                    <p class="text-danger text-center">Không tìm thấy thời khóa biểu để chỉnh sửa!</p>
